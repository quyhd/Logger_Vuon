...
{
    public partial class frmHistoryTOC : Form
    {
        ...
        /// contructor
        /// </summary>
        public frmHistoryTOC(LanguageService _lang)
        {
            InitializeComponent();
            //res_man = obj_res_man;
            //cul = obj_cul;
        ...
        /// <param name="e"></param>
        private void frmHistoryTOC_Load(object sender, EventArgs e)
        {
        ...
        private void reloadViewData(DataViewType _viewType, DataTimeType _timeType)
        {
            if (_viewType == DataViewType.List)
            {
                ...
                {
                    dt_source = db5m.get_all_toc(dtpDateFrom.Value, dtpDateTo.Value);
                }
                else //if (_timeType == DataTimeType._60Minute)
                {
                    dt_source = db60m.get_all_toc(dtpDateFrom.Value, dtpDateTo.Value);
                }
                ...
                foreach (DataRow row in dt_source.Rows)
                {
                    ...
                    viewrow["Status_Val"] = row["toc_status"];
                ...
                foreach (DataGridViewRow row in dgvData.Rows)
                {
                    ...
                    if (row.Cells["TOC"].Value != null)
                    {
                        if (Convert.ToDouble(row.Cells["TOC"].Value) < 0)
                        {
                            row.Cells["TOC"].Value = "---";
                        }
                    }
            ...
            else //if(_viewType == DataViewType.Graph)
            {
                ...
                foreach (DataRow row in dt_source.Rows)
                {
                    ...
                    int _status = (int)row["toc_status"];
                    if (_status == 0)
                    {
                        viewrow = dt_view.NewRow();
                ...
                chtData.Series["TOC"].Color = Color.Blue;
                ...
                chtData.ChartAreas[0].AxisY.Title = "TOC (mg/L)";
