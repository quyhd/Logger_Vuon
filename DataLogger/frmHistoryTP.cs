...
{
    public partial class frmHistoryTP : Form
    {
        ...
        /// contructor
        /// </summary>
        public frmHistoryTP(LanguageService _lang)
        {
            InitializeComponent();
            //res_man = obj_res_man;
            //cul = obj_cul;
        ...
        /// <param name="e"></param>
        private void frmHistoryTP_Load(object sender, EventArgs e)
        {
        ...
        private void reloadViewData(DataViewType _viewType, DataTimeType _timeType)
        {
            if (_viewType == DataViewType.List)
            {
                ...
                {
                    dt_source = db5m.get_all_tp(dtpDateFrom.Value, dtpDateTo.Value);
                }
                else //if (_timeType == DataTimeType._60Minute)
                {
                    dt_source = db60m.get_all_tp(dtpDateFrom.Value, dtpDateTo.Value);
                }
                ...
                foreach (DataGridViewRow row in dgvData.Rows)
                {
                    ...
                    if (row.Cells["pH"].Value != null)
                    {
                        if (Convert.ToDouble(row.Cells["pH"].Value) < 0)
                        {
                            row.Cells["pH"].Value = "---";
                        }
                    }
            ...
            else //if(_viewType == DataViewType.Graph)
            {
                ...
                chtData.Series["pH"].Color = Color.Blue;
                ...
                chtData.ChartAreas[0].AxisY.Title = "pH (mg/L)";
