...
{
    public partial class frmHistoryTN : Form
    {
        ...
        /// contructor
        /// </summary>
        public frmHistoryTN(LanguageService _lang)
        {
            InitializeComponent();
            //res_man = obj_res_man;
            //cul = obj_cul;
        ...
        /// <param name="e"></param>
        private void frmHistoryTN_Load(object sender, EventArgs e)
        {
        ...
        private void reloadViewData(DataViewType _viewType, DataTimeType _timeType)
        {
            if (_viewType == DataViewType.List)
            {
                ...
                {
                    dt_source = db5m.get_all_tn(dtpDateFrom.Value, dtpDateTo.Value);
                }
                else //if (_timeType == DataTimeType._60Minute)
                {
                    dt_source = db60m.get_all_tn(dtpDateFrom.Value, dtpDateTo.Value);
                }
                ...
                foreach (DataGridViewRow row in dgvData.Rows)
                {
                    ...
                    if (row.Cells["SS"].Value != null)
                    {
                        if (Convert.ToDouble(row.Cells["SS"].Value) < 0)
                        {
                            row.Cells["SS"].Value = "---";
                        }
                    }
