...
namespace DataLogger.Data
{
    public class data_5minute_value_repository : NpgsqlDBConnection
    {
        ...
        public int add(ref data_value obj)
        {
            using (NpgsqlDBConnection db = new NpgsqlDBConnection())
            {
                try
                {
                    ...
                    {
                        string sql_command = "INSERT INTO data_5minute_values (MPS_pH, MPS_pH_status, MPS_EC, MPS_EC_status, " +
                ...
                try
                {
                    ...
                    if (db.open_connection())
                    {
                        string sql_command = "UPDATE data_5minute_values set  " +
                                            " MPS_pH = :MPS_pH, MPS_pH_status =:MPS_pH_status, " +
                                            " MPS_EC =:MPS_EC, MPS_EC_status =:MPS_EC_status,  " +
                                            " MPS_DO =:MPS_DO, MPS_DO_status =:MPS_DO_status,  " +
                                            ...
                                            " module_PumpRFLT =:module_PumpRFLT, " +
                                            " module_air1 =:module_air1, module_air2 =:module_air2, module_cleaning =:module_cleaning, " +
        ...
        //            if (db.open_connection())
        //            {
        //                string sql_command = "DELETE from data_5minute_values where id = " + id;
                ...
                try
                {
                    if (db.open_connection())
                    {
                        string sql_command = @"SELECT stored_date, stored_hour, stored_minute, mps_ph,
                                                      mps_ec, mps_do, mps_turbidity,mps_orp, mps_ss, mps_temp, mps_status
                                               FROM data_5minute_values
                                               WHERE (:d_from < stored_date AND stored_date < :d_to)
                                                     OR
                                                     (stored_date = :d_from AND stored_date < :d_to   AND (stored_hour  > :h_from OR (stored_hour  = :h_from AND stored_minute >= :m_from)))
